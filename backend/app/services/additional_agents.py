...
class TelemetryDataProcessorAgent(BaseAgent):
    ...
    def __init__(self):
        super().__init__("TelemetryDataProcessor", "Processes lat/long/SOW telemetry and water current data")
    ...
    async def _execute_internal(self, context: WorkflowContext) -> Dict[str, Any]:
        ...
        return {
            ...
            'voyage_sets': {
                'away_from_dock': self._serialize_voyage_data(voyage_sets['away_from_dock']),
    ...
    async def _load_sample_water_current_data(self) -> pd.DataFrame:
        ...
        current_data = {
            ...
            'tide_height': np.sin(np.arange(n_points) * 2 * np.pi / 12.42) * 2 + 3  # Tidal pattern
        }
        
        return pd.DataFrame(current_data)
    
    async def _merge_telemetry_with_currents(self, telemetry: pd.DataFrame, currents: pd.DataFrame) -> pd.DataFrame:
        ...
        merged['current_direction'] = np.interp(
            merged.index,
            currents.index,
            currents['current_direction']
        )
        merged['tide_height'] = np.interp(
            merged.index,
            currents.index,
            currents['tide_height']
        )
        
        # Calculate speed over ground accounting for currents
        merged['speed_over_ground'] = merged['speed_over_water'] + np.random.normal(0, 0.5, len(merged))
        
        return merged
    
    async def _separate_voyage_sets(self, telemetry: pd.DataFrame) -> Dict[str, List[pd.DataFrame]]:
        ...
        telemetry['voyage_change'] = telemetry['is_docked'] != telemetry['is_docked'].shift(1)
        ...
        return {
            'away_from_dock': away_from_dock,
            'at_dock': at_dock
        }
    
    async def _generate_voyage_statistics(self, voyage_sets: Dict[str, List[pd.DataFrame]]) -> Dict[str, Any]:
        ...
        if away_voyages:
            ...
            distances = [v['speed_over_water'].sum() for v in away_voyages]  # Approximate nautical miles
            avg_speeds = [v['speed_over_water'].mean() for v in away_voyages if len(v) > 0]
    ...
    def _serialize_voyage_data(self, voyage_list: List[pd.DataFrame]) -> List[Dict[str, Any]]:
...
class VoyageAnalyticsEngine(BaseAgent):
    ...
    def __init__(self):
        super().__init__("VoyageAnalyticsEngine", "Calculates baseline fuel and emissions by voyage")
    ...
    async def _execute_internal(self, context: WorkflowContext) -> Dict[str, Any]:
        ...
        return {
            ...
            'csv_outputs': {
                'voyage_data': voyage_csv,
                'dwell_data': dwell_csv
            },
            'emissions_summary': await self._calculate_emissions_summary(voyage_analytics, dwell_analytics)
        }
    
    async def _calculate_voyage_analytics(self, profile_data: Dict, curves_data: Dict, telemetry_data: Dict) -> Dict[str, Any]:
        """Calculate fuel consumption and emissions for each voyage"""
        
        # Get performance curves
        fuel_curve = curves_data.get('performance_curves', {}).get('speed_vs_fuel_rate', {})
        power_curve = curves_data.get('performance_curves', {}).get('speed_vs_shaft_power', {})
        ...
        for i, voyage in enumerate(voyage_sets):
            ...
            voyage_results.append({
                ...
                'fuel_efficiency_nmpg': round(total_distance / max(total_fuel, 0.1), 2)
            })
        ...
        return {
            ...
            'aggregate_totals': {
                'total_fuel_gallons': sum(v['fuel_consumed_gallons'] for v in voyage_results),
                'total_distance_nm': sum(v['distance_nm'] for v in voyage_results),
                'total_operating_hours': sum(v['duration_hours'] for v in voyage_results),
                'total_co2_lbs': sum(v['co2_emissions_lbs'] for v in voyage_results)
            }
        }
    
    async def _calculate_dwell_analytics(self, profile_data: Dict, telemetry_data: Dict) -> Dict[str, Any]:
        ...
        for i, dwell in enumerate(dwell_sets):
            ...
            refuel_amount = min(fuel_consumed_during_dwell * 1.1, estimated_tank_capacity)  # 10% buffer
            ...
            dwell_results.append({
                ...
                'auxiliary_fuel_consumed_gallons': round(fuel_consumed_during_dwell, 2),
        ...
        return {
            ...
            'aggregate_totals': {
                ...
                'total_auxiliary_fuel_gallons': sum(d['auxiliary_fuel_consumed_gallons'] for d in dwell_results),
                'total_refuel_gallons': sum(d['estimated_refuel_amount_gallons'] for d in dwell_results)
            }
        }
    ...
    async def _calculate_emissions_summary(self, voyage_analytics: Dict, dwell_analytics: Dict) -> Dict[str, Any]:
        ...
        total_fuel = voyage_totals.get('total_fuel_gallons', 0) + dwell_totals.get('total_auxiliary_fuel_gallons', 0)
        total_co2 = voyage_totals.get('total_co2_lbs', 0) + (dwell_totals.get('total_auxiliary_fuel_gallons', 0) * 22.38)
        ...
        return {
            ...
            'auxiliary_fuel_gallons': dwell_totals.get('total_auxiliary_fuel_gallons', 0),
            'fuel_breakdown_percentage': {
                'operational': round((voyage_totals.get('total_fuel_gallons', 0) / max(total_fuel, 0.1)) * 100, 1),
                'auxiliary': round((dwell_totals.get('total_auxiliary_fuel_gallons', 0) / max(total_fuel, 0.1)) * 100, 1)
            }
        }
...
class ReportChartGeneratorAgent(BaseAgent):
    ...
    def __init__(self):
        super().__init__("ReportChartGenerator", "Creates charting and reports in frontend-compatible format")
    ...
    async def _execute_internal(self, context: WorkflowContext) -> Dict[str, Any]:
        ...
        all_outputs = {}
        for agent_name in ['BoatImageAnalyzer', 'BoatProfileBuilder', 'PerformanceCurveGenerator', 
                          'TelemetryDataProcessor', 'VoyageAnalyticsEngine']:
    ...
    async def _generate_charts(self, all_outputs: Dict) -> Dict[str, Any]:
        ...
        curves_data = all_outputs.get('PerformanceCurveGenerator', {}).get('performance_curves', {})
        if curves_data:
            charts['performance_curves'] = await self._create_performance_curve_charts(curves_data)
        
        # Voyage analytics charts
        voyage_data = all_outputs.get('VoyageAnalyticsEngine', {})
        if voyage_data:
            charts['voyage_analytics'] = await self._create_voyage_analytics_charts(voyage_data)
        
        # Emissions breakdown chart
        emissions_data = voyage_data.get('emissions_summary', {})
        if emissions_data:
            charts['emissions_breakdown'] = await self._create_emissions_chart(emissions_data)
        
        return charts
    
    async def _create_performance_curve_charts(self, curves_data: Dict) -> Dict[str, str]:
        ...
        try:
            ...
            if fuel_curve:
                ...
                plt.title('Vessel Speed vs Fuel Consumption Rate', fontsize=14, fontweight='bold')
    ...
    async def _create_voyage_analytics_charts(self, voyage_data: Dict) -> Dict[str, str]:
        ...
        try:
            ...
            if voyage_details:
                ...
                plt.plot(voyage_ids, efficiency, 'o-', color='green', linewidth=2, markersize=8)
                ...
                plt.savefig(buffer, format='png', dpi=150, bbox_inches='tight')
                buffer.seek(0)
                charts['efficiency_by_voyage'] = base64.b64encode(buffer.getvalue()).decode()
                plt.close()
    ...
    async def _generate_comprehensive_report(self, all_outputs: Dict) -> Dict[str, Any]:
        ...
        # Extract key data
        boat_profile = all_outputs.get('BoatProfileBuilder', {}).get('normalized_profile', {})
        performance_curves = all_outputs.get('PerformanceCurveGenerator', {})
        telemetry_summary = all_outputs.get('TelemetryDataProcessor', {}).get('telemetry_summary', {})
        ...
        report = {
            ...
            'vessel_overview': {
                ...
                'physical_specifications': boat_profile.get('physical_specifications', {}),
            ...
            'performance_analysis': {
                ...
                'validation_status': performance_curves.get('curve_metadata', {}).get('validation_status'),
                'confidence_level': performance_curves.get('curve_metadata', {}).get('confidence_level')
            },
            'operational_analysis': {
                'data_period': telemetry_summary.get('date_range', {}),
                'total_voyages': telemetry_summary.get('voyage_count', 0),
                'total_operating_hours': voyage_analytics.get('voyage_analytics', {}).get('aggregate_totals', {}).get('total_operating_hours', 0),
                'total_distance_nm': voyage_analytics.get('voyage_analytics', {}).get('aggregate_totals', {}).get('total_distance_nm', 0)
            },
    ...
    async def _create_executive_summary(self, all_outputs: Dict) -> Dict[str, Any]:
        ...
        totals = voyage_analytics.get('voyage_analytics', {}).get('aggregate_totals', {})
        ...
        return {
            ...
            'operational_summary': {
                ...
                'average_fuel_efficiency_nmpg': round(totals.get('total_distance_nm', 0) / max(totals.get('total_fuel_gallons', 0.1), 0.1), 2)
            },
            'environmental_impact': {
                'total_fuel_consumption_gallons': emissions.get('total_fuel_consumption_gallons', 0),
                'total_co2_emissions_tons': emissions.get('total_co2_emissions_tons', 0)
            },
    ...
    async def _generate_key_findings(self, all_outputs: Dict) -> List[str]:
        ...
        if voyage_analytics:
            ...
            if breakdown.get('auxiliary', 0) > 20:
                findings.append(f"High auxiliary fuel consumption ({breakdown.get('auxiliary', 0):.1f}%) indicates potential for electrification")
            
            totals = voyage_analytics.get('voyage_analytics', {}).get('aggregate_totals', {})
            if totals.get('total_fuel_gallons', 0) > 0:
                efficiency = totals.get('total_distance_nm', 0) / totals.get('total_fuel_gallons', 1)
                if efficiency < 2:
                    findings.append("Lower fuel efficiency suggests optimization opportunities in operational patterns")
    ...
    async def _generate_recommendations(self, all_outputs: Dict) -> List[str]:
        ...
        if operational.get('auxiliary_systems'):
            recommendations.append("Evaluate auxiliary system electrification for dwell periods to reduce emissions")
        
        if operational.get('commercial_operation'):
            recommendations.append("Explore commercial incentives for fuel efficiency improvements and emission reductions")
