...
class BaseAgent:
    ...
    async def execute(self, context: WorkflowContext) -> Dict[str, Any]:
        ...
        try:
            ...
            error_result = {"error": str(e), "status": "failed", "agent": self.name}
...
class BoatImageAnalyzerAgent(BaseAgent):
    ...
    def __init__(self, github_pat: str, api_url: str, model: str):
        super().__init__(
            "BoatImageAnalyzer", "Analyzes boat images and extracts specifications"
        )
    ...
    async def _analyze_image(self, input_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        analysis_service = BoatAnalysisService(
            github_pat=self.github_pat, api_url=self.api_url, model=self.model
        )
        ...
        return {
            ...
            "suggestions_for_review": await self._generate_review_suggestions(
                enhanced_result
            ),
        }
    ...
    async def _enhance_with_similar_boats(
...
class BoatProfileBuilderAgent(BaseAgent):
    ...
    def __init__(self):
        super().__init__(
            "BoatProfileBuilder", "Creates normalized boat profile with usage patterns"
        )
    ...
    async def _execute_internal(self, context: WorkflowContext) -> Dict[str, Any]:
        ...
        normalized_profile = await self._build_normalized_profile(
            boat_specs, simulated_responses
        )
    ...
    async def _generate_usage_questions(
        self, boat_specs: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
    ...
    async def _build_normalized_profile(
        self, boat_specs: Dict[str, Any], usage_responses: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        return {
            ...
            "profile_metadata": {
                ...
                "data_sources": [
                    (
                        "image_analysis"
                        if "image_analysis" in str(boat_specs)
                        else "manual_input"
                    ),
                    "user_questionnaire",
                ],
    ...
    def _extract_profile_attributes(
        ...
        if physical.get("boat_type"):
            attributes.append(
                {"category": "Physical", "name": "Type", "value": physical["boat_type"]}
            )
            ...
            attributes.append(
                {
                    "category": "Operational",
                    "name": "Daily Hours",
                    "value": f"{operational['daily_operating_hours']} hrs",
                }
            )
...
class PerformanceCurveGeneratorAgent(BaseAgent):
    ...
    def __init__(self):
        super().__init__(
            ...
            "Generates performance curves using Holtrop-Mennen resistance prediction",
        )
    ...
    async def _generate_speed_fuel_curve(
        self, profile: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    async def _generate_speed_power_curve(
        self, profile: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    async def _generate_speed_thrust_curve(
        self, profile: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    async def _generate_acceleration_power_curves(
        self, profile: Dict[str, Any]
    ) -> Dict[str, Any]:
...
Focus on technical rigor and practical applicability for baseline profiling."""
    ...
    def _generate_csv_performance_data(
        ...
        while speed <= service_speed_ms:
            ...
            csv_lines.append(
                f"{speed:.2f},{thrust_N:.1f},{shaft_power_W:.0f},{battery_demand_W:.0f}"
            )
            speed += 0.25
...
"""
    ...
    async def _generate_display_curves(self, profile: Dict[str, Any]) -> Dict[str, Any]:
        ...
        acceleration_power_curves = await self._generate_acceleration_power_curves(
            profile
        )
