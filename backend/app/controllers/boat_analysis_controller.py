...
from app.services.boat_analysis_service import (
    ...
    def analyze_boat():
        try:
            ...
            try:
                ...
                return (
                    jsonify(
                        {"ERROR": f"Failed to save image: {str(e)}", "CODE": "IMG003"}
                    ),
                    500,
                )
            ...
            analysis_result = analysis_service.analyze_boat(
                image_base64, boat_brand, boat_model
            )

            processing_time = time.time() - start_time

            boat_analysis = BoatAnalysis.create_from_analysis(
                filename, boat_brand, boat_model, analysis_result
            )
            boat_analysis.processing_time = processing_time

            current_app.logger.info(
                f"Analysis completed in {processing_time:.2f}s for {filename}"
            )
            ...
            return jsonify({"ERROR": "Internal server error", "CODE": "SYS002"}), 500

    @staticmethod
    def _validate_request():
        """Validate the incoming request"""
        if "image" not in request.files:
            return jsonify({"ERROR": "No image file provided", "CODE": "REQ001"}), 400

        if not request.files["image"].filename:
            return jsonify({"ERROR": "No image file selected", "CODE": "REQ002"}), 400

        if not current_app.config.get("GITHUB_PAT"):
            return (
                jsonify({"ERROR": "AI service not configured", "CODE": "CFG001"}),
                500,
            )
